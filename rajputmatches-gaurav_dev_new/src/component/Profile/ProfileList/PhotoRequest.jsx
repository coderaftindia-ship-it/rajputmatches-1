...
function PhotoRequest() {
  ...
  const fetchData = async () => {
    try {
      ...
      setData(fetchedData);
      setProfiles(activeTab === "requestSent" ? fetchedData.photoReqSent : fetchedData.photoReqReceived);
  ...
  function RequestImageContainer({ profile, activeButton, status }) {
    ...
    const isPrivate = !!profile?.filesId?.isPrivate && status !== "accepted";
    ...
    if (isPrivate || photos.length === 0) {
      ...
      if (activeButton === "requestReceived") {
        ...
        if (status === "pending") {
          ...
          // For accepted status, show no action buttons
          return renderEmptyState(null);
        }
      }
    }
      ...
      <div className="row m-0 mb-1 p-0 bg-white">
        <div className="col-8 col-sm-9 col-md-10 d-flex p-0">
          {["requestSent", "requestReceived"].map((tab) => (
            ...
            >
              {tab === "requestSent" 
                ? `Request Sent (${data?.photoReqSent?.length || 0})` 
                : `Request Received (${data?.photoReqReceived?.length || 0})`}
            </div>
          ))}
        </div>
        <div
